...
### What the tool does
Load an order `.xlsx` → for each row whose EAN is a `4005556…` article, it **checks whether that image actually exists** on the CDN (loads it as an `<img>` — `onload`/`onerror`), then writes `=IMAGE("<cdn url>")` into the **Front view** column only where it exists. Rows with no published image (~40% of her range, e.g. SINCO) are left **empty** — no `#BLOCKED` error — ready for a manual screenshot; re-running also clears an error formula left by a prior run. It then hands the file back. Opened in **Excel 365 with internet**, the front picture appears in each cell. The formula is stored as `_xlfn.IMAGE(…)` — the `_xlfn.` prefix is **required** for post-2007 functions; without it Excel shows `=@IMAGE`/`#NAME?`. In **German** Excel it displays and evaluates as `=BILD(…)`. The **Side column K and everything else** (side-image drawings, rich data, the PIM add-in, styles) are copied through untouched. Same zip reader/writer + EAN→article logic as the shelf planner.
...
1. **Theme-per-tab behaviour — RESOLVED.** Nina chose to name shelf areas herself to match the Excel tabs. So the rule is: a tab routes to the shelf area whose theme **name matches**, compared **ignoring case and separators** (`normName` in `loadOrderGrids` collapses spaces/underscores/hyphens) — that fix was needed because her tabs use underscores (`Me_Time`) while shelves use spaces (`Me Time`), which previously misrouted products to the active theme. A single unmatched tab still falls back to the sole area (if only one) else the active theme.
2. **Side-image automation (waiting on PIM).** Needs the stable per-EAN+view delivery URL from ContentServ (§3). Once available, extend `rv-front-image-filler.html` to fill column K — the existence-check plumbing (below) is already the right shape to reuse.
